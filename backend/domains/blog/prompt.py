...
예시 톤:
- OpenAI API로 벨로그 초안을 자동 생성하는 앱을 만들어봤다 - BlogDraftAI
- FastAPI + OpenAI API + Flutter로 블로그 초안 자동화 앱 만들기
...
OpenAI API를 선택한 이유와 프롬프트 엔지니어링 방향을 설명합니다.
앱 클라이언트가 있다면 Flutter 같은 크로스플랫폼 선택 이유도 연결하고, 코드에서 확인되지 않으면 향후 클라이언트 확장 관점으로 다룹니다.
텍스트 기반 아키텍처 다이어그램을 포함합니다.

예시:
앱/웹 화면 -> FastAPI 서버 -> GitHub 코드 수집 -> OpenAI API -> 블로그 초안 응답 -> Velog 포맷으로 정리
